...
dash["B2"].value     = "CAPA KPI Dashboard"
...
LOGIC_CONTENT = [
    ...
    ("The 'Date closed' on the Capas tab is the official system sign-off",
     "Someone deliberately marked the CAPA closed on that date. It is the date the management system recognizes as closure.", False),
    ("Task completion dates are inputs, not closure",
     "A CAPA may have many tasks. The last task finishing does not mean the CAPA is formally reviewed and closed — there may be approval or verification steps in between.", False),
    ("Using task dates creates artificially short averages",
     "In our validation, using task dates vs. the official close date reduced Portugal's avg from 49.9 days to 23.3 days — a 53% undercount that would misrepresent performance.", False),
    ("Consistency and auditability",
     "The Capas tab Date closed is a single, unambiguous field. Task completion data requires assumptions (which tasks count? what if some are incomplete?) that introduce subjectivity.", False),
    ("Defensible to stakeholders",
     "When asked 'how did you calculate this?', the answer 'the date the CAPA was marked closed in the system' is cleaner than 'the latest task completion date across two sheets'.", False),

    ("HOW WE DETERMINE IF A CAPA IS CLOSED", "", True),
    ("CLOSED",
     "The 'Date closed' field on the main Capas tab has a date. No exceptions.", False),
    ("OPEN",
     "The 'Date closed' field on the main Capas tab is blank.", False),
    ("Status column",
     "NOT used. Status values like 'Afgesloten' (Closed) are ignored — only the date field matters.", False),

    ("HOW EACH METRIC IS CALCULATED", "", True),
    ("Avg days to close (2025)",
     "All CAPAs with Date closed in 2025, regardless of when they were opened. Formula: Date closed − Date of notification, averaged.", False),
    ("Avg days to close (2026)",
     "Same logic applied to CAPAs with Date closed in 2026.", False),
    ("Total closed in 2025 / 2026",
     "Count of CAPAs where the year of Date closed matches.", False),
    ("Currently open",
     "Count of CAPAs where Date closed on the Capas tab is blank.", False),
    ("Open > 90 days",
     f"Open CAPAs where: today − Date of notification > 90 days. Uses Date of notification as the start date — when the CAPA was originally raised.", False),

    ("DATA SOURCES", "", True),
    ("Input files",
     "All files matching 'export_CAPA *.xls' in the same folder as the script.", False),
    ("Sheets used",
     "'Capas' sheet: source of all metrics. 'Taken' sheet: loaded but Effective closed date is for reference only — not used in any KPI calculation.", False),
    ("To refresh",
     "Replace the export_CAPA *.xls files and run:  py capa_metrics.py", False),
]
...
# SAVE
# ─────────────────────────────────────────────────────────────────────────────
output_path = os.path.join(SCRIPT_DIR, "CAPA_Metrics_Report.xlsx")
wb.save(output_path)
